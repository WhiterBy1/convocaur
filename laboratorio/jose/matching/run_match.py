"""
Match docente ↔ convocatoria (piloto José).

Uso:
  cd convocaur/laboratorio/jose
  python matching/run_match.py --convocatorias 45,48,976 --top 10
  python matching/run_match.py --sin-embeddings   # solo TF-IDF
"""
...
from cargar_datos import guardar_salida  # noqa: E402
from convocaur.paths import JSON_PROFESORES, PROC_NLP  # noqa: E402
from corpus import cargar_docentes_json, cargar_nlp_dir, texto_docente  # noqa: E402
from embedders import OpenRouterEmbedder, TfidfEmbedder, _load_env  # noqa: E402
from ranker import rankear_convocatoria  # noqa: E402
...
def main() -> None:
    ...
    conv_keys = [f"convocatoria_{c}" if not c.startswith("convocatoria_") else c for c in convs]

    out_dir = JOSE / "salidas" / "matching"
    ...
    for key in conv_keys:
        ...
        # también vía helper del lab
        guardar_salida("jose", f"matching/{csv_name}", df)

        top = df.head(3)[["rank", "nombre", "facultad", "categoria", "score_final", "score_emb", "score_tfidf"]]
