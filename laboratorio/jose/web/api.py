"""
API mínima para explorar el matching docente ↔ convocatoria (lab José).
...
JOSE = Path(__file__).resolve().parent.parent
LAB = JOSE.parent
CONVOCAUR = LAB.parent
MATCHING = JOSE / "matching"
SALIDAS = JOSE / "salidas" / "matching"
STATIC = Path(__file__).resolve().parent / "static"

sys.path.insert(0, str(MATCHING))
sys.path.insert(0, str(CONVOCAUR / "src"))

from convocaur.paths import JSON_PROFESORES, PROC_ELEGIBILIDAD, PROC_NLP  # noqa: E402
...
app = FastAPI(title="ConvocaUR Matching — José", version="0.1.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def _cache_stats() -> dict:
    cache_dir = SALIDAS / "cache_embeddings"
...
def flujo() -> dict:
    return {
        ...
        "pasos": [
            ...
            {"id": 6, "nombre": "Output ranking", "detalle": "Top-k CSV en salidas/matching/"},
        ],
        "notas": [
            "No reemplaza elegibilidad institucional (alianza, sede, grupos A1).",
            "Los dumps viven solo en laboratorio/jose/salidas/ (gitignored).",
        ],
    }
...
def listar_convocatorias() -> list[dict]:
    ...
    for csv_path in sorted(SALIDAS.glob("ranking_convocatoria_*.csv")):
        key = csv_path.stem.replace("ranking_", "")  # convocatoria_45
        meta = by_key.get(key, {})
...
def detalle_convocatoria(conv_id: str) -> dict:
    ...
    # quitar elegibilidad embebida enorme si estorba; la servimos aparte
...
def _boost_detalle(row: dict) -> list[dict]:
    ...
    # Señales temáticas del JSON del profesor (si existe)
    pid = row.get("id")
    if pid:
...
def _grafo_match(conv_id: str, numero: str, records: list[dict]) -> dict:
    ...
    for r in records:
        ...
        # Nodos-hoja de aportes que sí suman al score (raíces explicativas)
        for feat in caracts:
...
def ranking_convocatoria(conv_id: str, top: int = 20) -> dict:
    ...
    if not path.exists():
        raise HTTPException(404, f"No hay ranking para {conv_id}. Corre matching/run_match.py")
